...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            dmrsifftvec = dmrsslotgridnew(:,i)./DMRSSlotGridpos(:,i);
            ...
            r_l_dmrs(i,:,counter1) = abs(ifft(dmrsifftvec,ifftsizedmrs*dmrsifftscaler));
            [amount_dmrs, ind_dmrs] = max(r_l_dmrs(i,:,counter1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    PAPR_tot = 10*log10((max(PAPR_RE)/(sum(Slotpower)/NSlots)));
            ...
            PRSsymbolduration = (sum(ofdmInfo.SymbolLengths) + sum(ofdmInfo.CyclicPrefixLengths))/ofdmInfo.SampleRate;
            ...
            doppler_span = repmat(doppler,prs.NumRB*12,1);
          ...
          % Concatinating and removing the zero elements of row of slotGridPRSnew
            for i=1:(prs.NumRB*12)
                ...
                fftscaler = 1;
                v_d = abs(fft(fftvector,fftsize*fftscaler));
                [amount_v, ind_v] = max(v_d);
                ...
                speedesterror(i) = abs(speedest(i) - V_bis);
